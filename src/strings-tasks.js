...
function findLongestWord(sentence) {
  ...
  const numbers = array.map((item) => item.length);
  const max = Math.max(...numbers);
  return array[numbers.indexOf(max)];
}
...
function getCardId(value) {
  ...
  const cardSuits = ['♣', '♦', '♥', '♠'];
  let firstInd = cardDeck.indexOf(value[0]);
  let lastInd = cardSuits.indexOf(value[1]);
  if (firstInd === -1) {
    firstInd = cardDeck.indexOf('10');
    lastInd = cardSuits.indexOf(value[2]);
  }
  return firstInd + 13 * lastInd;
}
